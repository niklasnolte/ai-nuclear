...
# %%
# TRAIN MODEL
def train_model(epochs=100, hidden_dim=128, lr=1e-2, weight_decay=1, reg=1, amsgrad=True):
    torch.manual_seed(0)
    ...
    optimizer = mup.MuAdam(param_groups, lr=lr, amsgrad=True)
    ...
    for epoch in bar:
        ...
        # out_reg = forward_with_reg(model, random.randint(1,hidden_dim-1), X[mask_train])
        loss_train = criterion(out, y[mask_train].long())
        # loss_train += reg * criterion(out_reg, y[mask_train].long())
        loss_train.backward()
        optimizer.step()
...
for i, op in enumerate(operations):
    ax.text(embs[P + i, 0] , embs[P + i, 1]* 2.15 * (i+1), op, color="k", fontsize=8)
