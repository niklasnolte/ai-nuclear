...
from train_arith_combined import train
...
class BasicModelSmallBoth(nn.Module):
  #predicts a+b and a-b mod LIMIT
...
def run_models(dir, test_sizes, run = 'both'):
  ...
  combined = ['test_apb_acc', 'test_amb_acc', 'test_acc', 'train_acc']
  ...
  for test_size in test_sizes:
          ...
          print(test_size)
          seed+=1
          title = f'BasicModelSmall_256dim_{test_size}ts_{seed}seed_{run}'
          results['title'].append(title)
          results['seed'].append(seed)
          results['test_size'].append(test_size)

          values = train(modelclass=BasicModelSmallBoth, 
                          lr=1e-2, 
                          wd=1e-3,
                          embed_dim=embed_dim, 
          ...
          if run == 'combined':
             for i in range(len(values)-1):
                value = values[i]
                results[combined[i]].append(value)
          elif run == 'both':
             for i in range(len(values)-1):
                value = values[i]
                results[both[i]].append(value)
