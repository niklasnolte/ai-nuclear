...
def calculate_loss(y_pred, y_actual, loss_fn, weights, functions):
    ...
    for i, fn in enumerate(functions):
        ...
        loss += weights[i]*loss_fn(y_pred_fn_resize, y_actual_fn_resize)
        loss += -torch.log(weights[i])
    return loss
...
def train(modelclass, functions, lr, wd, embed_dim, basepath, device, title, seed = 1, test_size = 0.2):
  ...
  weights = torch.ones((len(functions),)).to(device).requires_grad_()
  parameters = [p for p in model.parameters()] + [weights]
  ...
  for i in bar:
    ...
    for X_batch, y_batch in train_loader:    
        ...
        train_loss = calculate_loss(y_batch_pred, y_batch, loss_fn, weights, functions)
        train_loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        ...
        loss += calculate_loss(y_test_pred, y_test, loss_fn, weights, functions)

        y_train_pred = model(X_train).to(device)
        train_loss += calculate_loss(y_train_pred, y_train, loss_fn, weights, functions)
        ...
        weight_vals = np.array([w.cpu().numpy() for w in weights])
        for i, fn in enumerate(functions):
            metrics[fn+'_weight'] = weight_vals[i]
        if loss < lowest_loss:
            lowest_loss = loss
