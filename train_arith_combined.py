...
def get_data(test_size = 0.2, seed = 42, run = 'combined'):
    ...
    if run == 'combined':
        ...
        train_mask = torch.rand(ys.shape[0])>test_size
        train_mask = train_mask.reshape((train_mask.shape[0], 1))
        train_mask = torch.hstack((train_mask, train_mask))
...
def get_y_pred(y_pred, train_mask):
   shape = y_pred.shape
   y_pred = y_pred.reshape((shape[0], 2, shape[1]//2))
   y_pred = y_pred[train_mask]
   return y_pred

def train(modelclass, lr, wd, embed_dim, basepath, device, title, seed = 1, test_size = 0.2, run = 'combined'):
  #train just for modular arithmetic. gets rid of norm and assums regtype is always simn
  torch.manual_seed(seed)
  os.makedirs(basepath, exist_ok=True)
  X, y, train_mask, test_mask, vocab_size = get_data(test_size = test_size, seed = seed, run = run) 
  ...
  for i in bar:
    ...
    with torch.no_grad():
      ...
      amb_mask = torch.hstack((torch.zeros(shape[0], 1), torch.ones(shape[0], 1))).bool()
      ...
      bar.set_postfix(loss=test_loss.item(), train_loss=train_loss.item(), test_apb_acc = test_apb_acc, test_amb_acc = test_amb_acc, test_acc = test_acc, ent=entropy)
      ...
      if test_acc > epsilon:
         ...
         return test_apb_acc, test_amb_acc, test_acc,train_acc, i
