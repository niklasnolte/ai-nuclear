...
class ResModel(nn.Module):
    def __init__(self,functions, hidden_dim=64, num_layers=2):
        ...
        self.P = config.LIMIT
        self.emb_a = nn.Embedding(self.P, hidden_dim)
        self.nonlinear = torch.nn.Sequential(
...
def train(modelclass, function_dict, lr, wd, basepath, device, title, embed_dim = 64, seed = 1, test_size = 0.2):
  ...
  X_train, X_test, y_train, y_test, vocab_size = get_data_nomod(functions, test_size = test_size, seed = seed) 
  ...
  for i in bar:
    ...
    with torch.no_grad():
        ...
        for j, fn in enumerate(fn_names):
            metrics[fn+'_train'] = loss_fn(y_train_pred[:,j], y_train[:,j]).cpu().numpy()
            metrics[fn+'_test'] = loss_fn(y_test_pred[:,j], y_test[:,j]).cpu().numpy()
